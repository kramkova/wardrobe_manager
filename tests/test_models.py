import unittest
from datetime import date
from src.models import Category, Season, Item, Outfit


class TestItemConstructor(unittest.TestCase):
    def setUp(self):
        self.item = Item(item_id=1, name='синяя рубашка', colour='синий', category=Category.TOP)

    def test_item_attributes_type(self):
        """ Тестируем типы атрибутов Item """

        self.assertIsInstance(self.item.id, int)
        self.assertIsInstance(self.item.name, str)
        self.assertIsInstance(self.item.category, Category)
        self.assertIsInstance(self.item.colour, str)
        self.assertIsInstance(self.item.season, Season)
        self.assertIsInstance(self.item.start, date)
        self.assertIsInstance(self.item.wear_count, int)

    def test_item_init(self):
        self.assertEqual(self.item.id, 1)
        self.assertEqual(self.item.name, 'синяя рубашка')
        self.assertEqual(self.item.category, Category.TOP)
        self.assertEqual(self.item.colour, 'синий')
        self.assertEqual(self.item.season, Season.ALLSEASON)
        self.assertEqual(self.item.start, date.today())
        self.assertEqual(self.item.wear_count, 0)
        self.assertEqual(self.item.brand, None)
        self.assertEqual(self.item.price, None)


class TestOutfitConstructor(unittest.TestCase):

    def setUp(self):
        """Подготовка данных для тестов"""
        self.shirt = Item(1, "Рубашка", Category.TOP, "синий")
        self.jeans = Item(2, "Джинсы", Category.BOTTOM, "черный")
        self.items = [self.shirt.id, self.jeans.id]

    def test_outfit_init(self):
        """Тестируем корректную инициализацию атрибутов Outfit"""
        outfit = Outfit(outfit_id=1, items=self.items, wear_date=date.today())

        self.assertEqual(outfit.id, 1)
        self.assertEqual(outfit.items, self.items)
        self.assertEqual(outfit.date, date.today())


if __name__ == '__main__':
    unittest.main()
